...
schemes should be considered unsafe for clinical deployment.

\vspace{0.4em}
\noindent\textbf{Code, data, and raw results:}
\url{https://github.com/UditAkhourii/bdp-clinical-rag} (MIT licence).
The corpus generator is deterministic from \texttt{SEED=42}; every
number in this paper is regenerable byte-for-byte with one command
(\texttt{python -m corpus \&\& python -m run \&\& python -m ablate}).
Raw per-pipeline metrics: \texttt{results/results.json},
\texttt{results/ablation.json},
\texttt{results/clinical\_accuracy.json}.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{figures/fig1_asymmetry.png}
\caption{Asymmetric vs.\ symmetric write/read pipelines. \emph{Top}: under
read-only pseudonymization the indexed corpus is embedded raw while the
query is tokenized; the document and query embeddings land in
non-overlapping regions of the embedding space and cosine similarity falls
(retrieval miss), and the indexed corpus still leaks raw PHI to the
embedding API at ingest time. \emph{Bottom}: under BDP both sides are
tokenized identically; the embeddings overlap and retrieval succeeds, with
no PHI crossing the trust boundary.}
\label{fig:asymmetry}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{figures/fig2_architecture.png}
\caption{BDP architecture. Identifier detection, HMAC-keyed deterministic
tokenization, format-preserving decoders, and the mapping vault all live in
the controller-operated trusted zone. The format-preserving decoder is the
only component that emits text across the trust boundary to the external
embedding and LLM APIs. The return path detokenizes the LLM response via
the vault so that authorized downstream consumers (clinicians) see real
identifiers.}
\label{fig:bdp}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{figures/fig3_recall.png}
\caption{Retrieval recall@$k$ across the four pipelines, bootstrap
reproduction ($n{=}100$ queries; MiniLM-L6-v2 embeddings). BDP
(highlighted) preserves most of Raw's retrieval signal while keeping zero
identifier leakage; Redaction and Read-only collapse to near-zero across
all $k$. Full-scale plot with 95\% bootstrap CIs over 1{,}000 resamples to
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{figures/fig4_trace.png}
\caption{End-to-end trace of one BDP query. The original clinician
question is tokenized at the trust boundary; the embedding and LLM APIs
see only the tokenized form (no real names, MRNs, or ABHA IDs); the LLM
returns a tokenized answer; and the vault detokenizes that answer at the
controller boundary so the clinician sees the real patient's identity.
Real PHI appears only in line~24, inside the trusted zone.}
\label{fig:trace}
\end{figure}
...
The reference implementation and Python eval harness described in
Section~\ref{sec:eval} are released at
\url{https://github.com/UditAkhourii/bdp-clinical-rag} under the MIT
licence. The repository contains:

\begin{itemize}
  \item \textbf{BDP reference implementation}: \texttt{bdp.py}
        (HMAC-keyed deterministic tokenization, seven category-specific
        format-preserving decoders, in-memory vault, lookup detector,
        redaction baseline).
  \item \textbf{Synthetic corpus generator}: \texttt{corpus.py}
        (deterministic from \texttt{SEED=42}; no LLM calls; emits patient
        roster, longitudinal notes, and category-stratified queries with
        ground-truth annotations).
  \item \textbf{Four-pipeline retrieval benchmark}: \texttt{pipelines.py}
        and \texttt{run.py} (produces Table~\ref{tab:main} and the
        retrieval columns of Table~\ref{tab:bycategory}).
  \item \textbf{Ablation runner}: \texttt{ablate.py} (produces
        Table~\ref{tab:ablation}).
  \item \textbf{Clinical-accuracy grader}: \texttt{clinical\_accuracy.py}
        with Gemini / Anthropic / OpenAI backends (produces the clinical
        accuracy columns of Tables~\ref{tab:main} and
        \ref{tab:bycategory}).
  \item \textbf{Dependency-free correctness test}: \texttt{smoke\_test.py}
        (verifies determinism, format preservation, vault roundtrip,
        asymmetric divergence, and redaction collapse using only the
        Python standard library).
\end{itemize}

The full benchmark regenerates byte-for-byte with:

\begin{center}
\texttt{python -m corpus \&\& python -m run \&\& python -m ablate}
\end{center}

The raw per-pipeline result files committed to the repository at the time
of this preprint are:

\begin{itemize}
  \item Retrieval benchmark (Table~\ref{tab:main}, retrieval columns):
        \url{https://github.com/UditAkhourii/bdp-clinical-rag/blob/main/eval/results/results.json}
  \item Ablation study (Table~\ref{tab:ablation}):
        \url{https://github.com/UditAkhourii/bdp-clinical-rag/blob/main/eval/results/ablation.json}
  \item Clinical-accuracy grader output (Table~\ref{tab:main} accuracy
        columns; Table~\ref{tab:bycategory} accuracy columns):
        \url{https://github.com/UditAkhourii/bdp-clinical-rag/blob/main/eval/results/clinical_accuracy.json}
\end{itemize}

Human-readable markdown summaries of the same files
(\texttt{summary.md}, \texttt{ablation.md},
\texttt{clinical\_accuracy.md}) are also committed in the same directory.

\section*{Author Contributions}

This is a single-author preprint. U.R.A.\ conceived the failure-mode
framing; designed the BDP protocol and its formal model
(Section~\ref{sec:asymmetry}); implemented the reference SDK and the
Python eval harness; ran the bootstrap reproduction including the
four-pipeline benchmark, ablation study, and clinical-accuracy grading;
and drafted the manuscript. A clinical informaticist co-author and an
ML/privacy methodologist co-author are being recruited and will be added
at the first preprint revision alongside the full-scale evaluation run.

\section*{Conflicts of Interest}

The author is the founder of Brane Labs, a company that commercializes
compliance tooling for healthcare AI startups. The BDP reference
implementation is a subset of Brane Labs' open-source SDK. The synthetic
corpus and evaluation protocol contain no proprietary content, and the
released code is independently runnable.
